...
## ECS 性能实验室 —— 四路实现任意两两对比(同一逻辑: 移动+治疗)
##
## 四路"使用方式"做完全相同的逻辑:
##   A. 手写脚本层 : ECS + GDScript 循环(HealSystem)
##   B. 声明规则层 : ECS + 声明式规则(HealRule, C++ 批量)
##   C. 原生API层  : ECS + 直接 batch 调用(C++ 批量)
##   D. 普通Node实现: 传统 OOP, 每实体一个 Node2D + _process
##
## 每路有固定颜色, 可任意两两对比, 每帧显示各自耗时与加速比。
## 运行: F5 运行 Scenes/ECS/ECSPerfLab.tscn

@export var entity_count: int = 50000     # 每路实体数
...
var compare_l: String = "声明规则层"
var compare_r: String = "原生API层"

## 每模式固定颜色
const MODE_COLORS := {
	"手写脚本层": Color(0.3, 1.0, 0.5),    # 绿
	"声明规则层": Color(0.3, 0.8, 1.0),    # 青
	"原生API层": Color(0.9, 0.5, 1.0),     # 紫
	"普通Node实现": Color(1.0, 0.6, 0.2),  # 橙
}
...
func _setup_all() -> void:
	...
	for mode in MODES:
		...
		for i in entity_count:
			...
			w.add_component(e, ECSDemoMoveComponent)
			var angle := randf() * TAU
			w.set_field(e, ECSDemoMoveComponent, &"pos",
					Vector2(randf_range(20.0, 1130.0), randf_range(20.0, 700.0)))
			w.set_field(e, ECSDemoMoveComponent, &"vel",
					Vector2(cos(angle), sin(angle)) * randf_range(20.0, 50.0))
	...
	for i in entity_count:
		...
		unit.vel = Vector2(cos(angle), sin(angle)) * randf_range(20.0, 50.0)
		unit.hp = i % 100
...
func _apply_comparison() -> void:
	# 左右路颜色/标题
	cloud_l.set_color(MODE_COLORS[compare_l])
	cloud_r.set_color(MODE_COLORS[compare_r])
	...
	for u in _plain_nodes:
		...
		# 普通Node未选中时不渲染(避免干扰画面)
		u.visible = plain_selected
	# 普通Node未选中时, 两个点阵只画 ECS 路
	_update_stats()
...
func _process(delta: float) -> void:
	...
	if _frame % 3 == 0:
		...
		_bounce_mode(compare_l)
		_bounce_mode(compare_r)
		_update_stats()
...
func _bounce_mode(mode: String) -> void:
	if _ecs_worlds.has(mode):
		_bounce_ecs(_ecs_worlds[mode])
	elif mode == "普通Node实现":
		for u in _plain_nodes:
			u.bounce()
