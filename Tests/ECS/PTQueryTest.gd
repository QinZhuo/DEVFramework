...
static func run() -> void:
	...
	# ---- 规则动作 mul ----
	var q2 = load("res://addons/DEVFramework/Entity/ECS/ECSRuleQuery.gd").new()
	q2._init_rule(w, PTCompA)
	q2.mul(&"x", 2.0)
	q2.execute()
	print("[Query] mul_1=", int(w.get_field(ids[1], PTCompA, &"x")))   # 1*2=2
	# div: ids[4] 原 4 → mul 8 → div 4 → 2
	var q3 = load("res://addons/DEVFramework/Entity/ECS/ECSRuleQuery.gd").new()
	...
	var q4 = load("res://addons/DEVFramework/Entity/ECS/ECSRuleQuery.gd").new()
